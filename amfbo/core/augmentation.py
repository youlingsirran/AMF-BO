...
def aug_random_walk(graph, root, drop_percent=0.2):
    ...
    num_edge = rg.number_of_edges()
    retain_num = num_edge - int(num_edge * drop_percent)
...
def aug_loss_modality(graph, drop_percent=0.2):
    ...
    aug_graph = add_self_loop_if_not_in(aug_graph)  # aug_graph = add_self_loop_if_not_in(aug_graph)
    return aug_graph
