...
from amfbo.extraction.utils.time_utils import *
...
def process_traces(dir):
    ...
    trace_df.to_csv(f"D:/data/GAIA-DataSet-main/MicroSS/trace.csv")
    return trace_df
...
def process_logs(dir):
    ...
    log_df.to_csv("D:/data/GAIA-DataSet-main/MicroSS/process_log.csv")
    return log_df
...
def read_all_metrics():
    ...
    for f in os.listdir("D:/data/GAIA-DataSet-main/MicroSS/metric/metric_split/metric"):
        print(f)
        splits = f.split('_')
        cur_pod, cur_host = splits[0], splits[1]
        if (cur_pod not in pod_names) or ('2021-07-15_2021-07-31' in f) or ('2021-08' in f):
            continue
        metric_name = '_'.join(splits[2:-2])
        df1 = pd.read_csv(f'D:/data/GAIA-DataSet-main/MicroSS/metric/metric_split/metric/{f}')
        next_name = f.replace(
                        "2021-07-01_2021-07-15",
                        "2021-07-15_2021-07-31"
                    )
        df2 = pd.read_csv(f'D:/data/GAIA-DataSet-main/MicroSS/metric/metric_split/metric/{next_name}')
...
if __name__ == '__main__':
    # trace_df = process_traces("D:/data/GAIA-DataSet-main/MicroSS/trace/trace_split/trace")
    log_df = process_logs("D:/data/GAIA-DataSet-main/MicroSS/business/business_split/business")

    label_df = pd.read_csv("D:/data/GAIA-DataSet-main/MicroSS/gaia.csv")
    ...
    # io_utils.save("MicroSS/pre-data.pkl", pre_data)
    io_utils.save("D:/data/GAIA-DataSet-main/MicroSS/post-data-10_log.pkl", post_data)
    io_utils.save("D:/data/GAIA-DataSet-main/MicroSS/pre_data-10_log.pkl", pre_data)